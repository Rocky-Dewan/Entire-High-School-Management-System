...
if(isset($_POST["do"])&&($_POST["do"]=="add_dkash")){
	...
	if($received_type == "Admin"){
		...
		$sql1 = "INSERT INTO dkash(received_by,approved_by,year,month,date,time,paid,received_type,_status)
		...
		if(mysqli_query($conn,$sql1)){
			...
			$sql2="SELECT id FROM dkash ORDER BY id DESC LIMIT 1;";
		...
		for($i=0;$i<count(($_POST['_desc']));$i++){
			...
			$sql = "INSERT INTO dkash_history(_desc,received_by,approved_by,year,month,date,time,amount,total_paid,invoice_number,received_type,_status)
	...
	if($received_type == "Teacher"){
		...
		$sql1 = "INSERT INTO dkash(received_by,year,month,date,time,paid,received_type,_status)
		...
		for($i=0;$i<count(($_POST['_desc']));$i++){
			...
			$sql = "INSERT INTO dkash_history(_desc,received_by,year,month,date,time,amount,total_paid,invoice_number,received_type,_status)
	...
	if(isset($_POST["user_type"])&&($_POST["user_type"]=="Admin")){
		header("Location: view/dkash.php?do=alert_from_insert&msg=$msg");//MSK-000143-5
	}
	
	if(isset($_POST["user_type"])&&($_POST["user_type"]=="Teacher")){
		header("Location: view/my_dkash.php?do=alert_from_insert&msg=$msg");//MSK-000143-5
	}
