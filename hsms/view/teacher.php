...
$("#form1").submit(function (e) {
	...
	if(phone == ''){
		...
		if (telformat.test(phone) == false){ 
			...
			$("#phone" ).keydown(function(){//RD-00105-phone
	...
	if(email == ''){
		...
		if (mailformat.test(email) == false){ 
			...
			$("#email").keydown(function() {//RD-00109-email
	...
	if(full_name == '' || i_name == '' || address == '' || gender == '' || phone == '' || email == '' || mailformat.test(email) == false || telformat.test(phone) == false || photo == '' ){
		//RD-000098- form1 validation failed
		$("#btnSubmit").attr("disabled", true);
		e.preventDefault();
		return false;
